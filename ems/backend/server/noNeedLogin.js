...
module.exports = {
    ...
    newsList: async ctx => {   //新闻和活动
        ...
        //isuse要写在前面，and的优先级要更高
        const sql = 'select id,title,intro,picUrl,hits,upTime,path from article where isUse="true" and (fid=' + newsid + ' or fid=' + activeid + ') order by id desc limit 8';
        const result = await mysql.nquery(sql);
        ctx.body = result
    },
