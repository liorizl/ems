...
module.exports = {
    ...
    upSetting: async ctx => {
        await serverUtil.getForm(ctx.req).then(async value => {
            ...
            let sql = act === 'edit' ? 
                'update webSetting set webName="' + value.webName[0] + '", webUrl="' + value.webUrl[0] + '", keyword="' + value.keyword[0] + '", description="' + value.description[0] + '", ' +
                ...
                : 
                'insert into webSetting(webName, webUrl, keyword, description, extendName, indexModel, pageModel, author, indexPath, pagePath, buildCol, buildFaCol, buildArt, buildFaArt, listNum) ' +
                'values("' + value.webName[0] + '", "' + value.webUrl[0] + '", "' + value.keyword[0] + '", "' + value.description[0] + 
                '", "' + value.extendName[0] + '", ' + parseInt(value.indexModel[0]) + ', ' + parseInt(value.pageModel[0]) + ', "' + value.author[0] + 
                '", "' + value.indexPath[0] + '", "' + value.pagePath[0] + '", ' + parseInt(value.buildCol[0]) + ', ' + parseInt(value.buildFaCol[0]) + ', ' + parseInt(value.buildArt[0]) + 
                ', ' + parseInt(value.buildFaArt[0]) + ', ' + value.listNum[0] + ')';
