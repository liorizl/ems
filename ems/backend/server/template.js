...
module.exports = {
    ...
    getNewsMes: async ctx => {
        const sql = 'select name, alias, isUse, keyword, intro, template, contentTemp from news_setting';
        const result = await mysql.nquery(sql);
        ctx.body = result
    },
    getNewsTemps: async ctx=> {
        const sqlPage = 'select title from template where type = 3';
        const sqlCon = 'select title from template where type = 4';
        const resPage = await mysql.nquery(sqlPage);
        const resCon = await mysql.nquery(sqlCon);
        ctx.body = {
            pageTemps: resPage,
            contentTemps: resCon
        }
