...
const util = {
    ...
    increaseIndent: (myStr, indentNum, i = null) => {    //给模版每一行增加缩进(str,缩进数，层数)
        ...
        if(typeof myStr !== 'string') {
            // 防止传过来的myStr不是字符串
            myStr = myStr.htmlC || '参数myStr不是字符串，请检查！';
        }
        arr = myStr.split(/\r\n/);
        arr = arr.map((str, index) => {
            if (i) {
    ...
    makePaging: (num, now, len, type, url, build = null, colids) => {
        ...
        if (type === 1) {
            ...
            if (pageCount > 0) {
                ...
                if (pageCount > 9) {  //当条数大于9条是要隐藏一部分
                    for (let i = 1; i <= pageCount; i++) {
                        if (i === n) {
                            html += new Array(8).join(' ') + '<li class="now">' + i + '</li>\r\n'
                        }
                        else if(i >= 4 && i < pageCount - 1 && n === i - 2) {
                            html += new Array(8).join(' ') + '<li>...</li>\r\n'
                            i = pageCount - 1
                        }
                        else if(i >= 2 && i <= n - 3) {
                            html += new Array(8).join(' ') + '<li>...</li>\r\n'
                        ...
                        else if(n === 1 && i > 2 && i < pageCount - 1 && i !== Math.floor(pageCount / 2)) {
                            const mid = Math.floor(pageCount / 2)
                            if (i > 2 && i < mid){
                                html += new Array(8).join(' ') + '<li>...</li>\r\n'
                                i = mid - 1
                            } else if (i < pageCount - 1 && i > mid){
                                html += new Array(8).join(' ') + '<li>...</li>\r\n'
                                i = pageCount - 2
                            }
                        }
